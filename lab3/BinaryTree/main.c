...
TreeNodePtr createTreeWithLevelOrder(int *data, int size) {
    ...
    else
    {
        ...
        TreeNodePtr BT,T;
        BT = createTreeNode(data[i],NULL,NULL);
        EnQueue(Q,BT);
        while(!QueueEmpty(Q))
        {
            T = GetHead(Q);
            ...
            if(++i < size)
            {
                ...
                {
                    T->left = NULL;
                    EnQueue(Q,createTreeNode(data[i],NULL,NULL));
                }
                else
                {
...
int maxPathSum(TreeNodePtr root, int sum) {
    ...
    {
        sum = sum + root->val;
        return maxPathSum(root->left,sum) > maxPathSum(root->right,sum) ? maxPathSum(root->left,sum) : maxPathSum(root->right,sum);
    }
}
...
TreeNodePtr invertTree(TreeNodePtr root) {
    ...
    TreeNodePtr T = createTreeNode(root->val,NULL,NULL);
